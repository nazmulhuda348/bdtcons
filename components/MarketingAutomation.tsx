...
import { 
  ...
  Loader2, Trash2, Smartphone, RefreshCw, Clock, Wifi, WifiOff, LogOut, Plus, ShieldCheck, Filter, Search
} from 'lucide-react';
...
export const MarketingAutomation: React.FC = () => {
  // 🔴 পরিবর্তন ১: leads এর বদলে allProspects নিয়ে আসা হয়েছে 🔴
  const { allProspects, projects } = useAppContext(); 
  ...
  // 🔴 পরিবর্তন ২: allProspects থেকে ক্যাটাগরি স্ক্যান করা হচ্ছে 🔴
  const categories = useMemo(() => {
    const leadCats = (allProspects || []).map(l => l.category).filter(Boolean);
    const projectNames = (projects || []).map(p => p.name);
    const allCats = new Set(['General', ...leadCats, ...projectNames]);
    return ['all', ...Array.from(allCats)];
  }, [allProspects, projects]);

  // 🔴 পরিবর্তন ৩: allProspects থেকে কন্টাক্ট ফিল্টার করা হচ্ছে 🔴
  const filteredLeadsList = useMemo(() => {
    return (allProspects || []).filter(l => {
      ...
      return hasPhone && catMatch && nameMatch;
    });
  }, [allProspects, selectedCategory, searchTerm]);
  ...
  useEffect(() => {
    ...
    socket.on('ready', () => { setIsConnected(true); setQrCode(null); setIsConnecting(false); setIsDisconnecting(false); });
    socket.on('disconnected', () => { setIsConnected(false); setQrCode(null); setIsConnecting(false); setIsDisconnecting(false); });
    fetchStatus();
  ...
  const fetchStatus = async () => {
    try {
      ...
      if (res.ok) {
        ...
        if (data.status === 'CONNECTED') { setIsConnected(true); setIsConnecting(false); setQrCode(null); }
        else if (data.status === 'QR_READY' && data.qr) { updateQrState(data.qr); }
        else { setIsConnected(false); setIsConnecting(false); setQrCode(null); }
  ...
  const toggleAll = () => {
    if (selectedLeads.length === filteredLeadsList.length && filteredLeadsList.length > 0) setSelectedLeads([]);
    else setSelectedLeads(filteredLeadsList.map(l => l.id));
  };
  ...
  const triggerConnection = async () => {
    ...
    try { await fetch(`${BACKEND_URL}/api/marketing/connect`, { method: 'POST' });
      setTimeout(() => { if (!qrCode && !isConnected) { setIsConnecting(false); fetchStatus(); } }, 10000);
  ...
  const handleLogout = async () => {
    ...
    try { await fetch(`${BACKEND_URL}/api/marketing/disconnect`, { method: 'POST' }); setIsConnected(false); setQrCode(null); }
  ...
  const launchCampaign = async () => {
    ...
    setIsSending(true); setSendingProgress(0);
    ...
    const mediaList = await Promise.all(mediaFiles.map(async (file) => {
      ...
      return { data: base64Data, mimetype: file.type, filename: file.name };
    }));
    try {
      const response = await fetch(`${BACKEND_URL}/api/marketing/send`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ recipients: targetPhones, message: finalMessage, mediaList })
      });
    ...
    } catch (err) { alert("Error: " + (err as Error).message); setIsSending(false); }
  };
              ...
              <motion.div key="composer" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-slate-800 rounded-[2.5rem] border border-slate-700 p-8 shadow-2xl space-y-8">
                <div className="space-y-2">
                  ...
                  <div className="flex items-start space-x-2 mt-2 px-2">
                    ...
                    <p className="text-[10px] text-slate-400 leading-tight">A professional signature will be added automatically.</p>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                     {mediaPreviews.map((preview, index) => (
                       <div key={index} className="aspect-square bg-slate-900 rounded-2xl relative overflow-hidden group border border-slate-700">
                          {mediaFiles[index]?.type.startsWith('video/') ? <video src={preview} className="w-full h-full object-cover" /> : <img src={preview} className="w-full h-full object-cover" />}
                          <div className="absolute inset-0 bg-slate-950/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity"><button onClick={(e) => removeFile(index, e)} className="bg-red-500 p-2 rounded-xl text-white hover:scale-110"><Trash2 size={16} /></button></div>
                       </div>
                     ))}
                     <div onClick={() => fileInputRef.current?.click()} className="aspect-square bg-slate-900/50 border-2 border-dashed border-slate-700 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-amber-400 transition-all"><ImageIcon className="text-slate-600 mb-1" size={24} /><span className="text-[8px] text-slate-500 font-bold uppercase">Upload</span></div>
                  </div>
          ...
          <div className="bg-slate-800 rounded-[2.5rem] border border-slate-700 shadow-2xl overflow-hidden flex flex-col h-[750px]">
            <div className="p-8 border-b border-slate-700 bg-slate-900/50 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4 w-full md:w-auto">
                ...
                <div>
                  <h3 className="text-xl font-bold text-white font-outfit leading-tight">Recipients</h3>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
                <div className="relative w-full sm:w-48">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-900 border border-slate-700 rounded-xl py-2.5 pl-10 pr-4 text-[10px] text-white focus:ring-2 focus:ring-amber-400/20 outline-none font-bold"
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                  />
                </div>

                <div className="flex items-center space-x-2 bg-slate-900 p-1.5 rounded-xl border border-slate-700 w-full sm:w-auto">
                  <div className="px-2 py-1 bg-slate-800 rounded-lg"><Filter size={12} className="text-amber-400" /></div>
                  <select 
                    value={selectedCategory}
                    onChange={(e) => { setSelectedCategory(e.target.value); setSelectedLeads([]); }}
                    className="bg-transparent text-white text-[10px] font-bold uppercase tracking-wider outline-none border-none pr-2 cursor-pointer w-full"
                  >
                    {categories.map(cat => (
                      <option key={cat} value={cat} className="bg-slate-800 text-white">{cat === 'all' ? 'All Segments' : cat}</option>
                    ))}
                  </select>
                </div>

                <button onClick={toggleAll} className="bg-slate-900 border border-slate-800 px-6 py-3 rounded-xl text-[10px] font-black text-slate-300 uppercase hover:text-white transition-all whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-slate-700/50">
                     {filteredLeadsList.length > 0 ? filteredLeadsList.map((lead) => (
                        <tr key={lead.id} onClick={() => toggleLead(lead.id)} className={`cursor-pointer transition-colors group ${selectedLeads.includes(lead.id) ? 'bg-amber-400/5' : 'hover:bg-slate-700/20'}`}>
                           ...
                           <td className="px-8 py-6"><span className="text-[9px] font-bold px-2 py-1 bg-slate-700 rounded text-amber-400 uppercase tracking-wider">{lead.category || 'General'}</span></td>
                           <td className="px-8 py-6 text-xs text-slate-500 font-mono">{lead.phone}</td>
                        </tr>
                     )) : (
                        <tr><td colSpan={4} className="text-center py-20 text-slate-600 font-bold italic tracking-wider">No matching leads found...</td></tr>
                     )}
                  </tbody>
               </table>
            </div>
