...
import { 
  ...
  MapPin, Briefcase, Heart, Tag, Edit2, FolderPlus, Check
} from 'lucide-react';
...
export const Leads: React.FC = () => {
  const { leads, updateLeads, updateClients, deleteLead, currentUser } = useAppContext();
            ...
            <tbody className="divide-y divide-slate-700">
              <AnimatePresence mode="popLayout">
                {filteredLeads.map((lead) => {
                  const canEdit = isAdmin || lead.createdByUserId === currentUser?.id;
...
const LeadModal: React.FC<{ onClose: () => void, editData?: Lead }> = ({ onClose, editData }) => {
  const { leads, updateLeads, currentUser } = useAppContext();
  ...
  // Extract unique categories based on existing leads data
  const availableCategories = useMemo(() => {
    const defaults = ['General', 'Land', 'Software', 'Interior', 'Construction'];
    const fromLeads = leads.map(l => l.category).filter(Boolean) as string[];
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
             <div className="flex justify-between items-center mb-1">
               ...
               <button type="button" onClick={() => setShowNewCategory(!showNewCategory)} className="text-[9px] font-black text-amber-400 uppercase tracking-widest hover:text-amber-300 flex items-center gap-1">
                  <FolderPlus size={12} /> {showNewCategory ? 'Cancel' : 'Add Category'}
               </button>
             </div>
             {showNewCategory ? (
                <div className="flex gap-2 animate-in slide-in-from-top-2 duration-200">
                  <input autoFocus placeholder="New Category Name..." className="flex-1 bg-slate-900 border border-amber-500/30 rounded-xl px-4 py-2 text-white outline-none focus:ring-1 focus:ring-amber-400" value={newCategory} onChange={e => setNewCategory(e.target.value)} />
                  <button type="button" onClick={handleAddCategory} className="bg-amber-400 text-slate-950 p-3 rounded-xl hover:bg-amber-300 transition-colors"><Check size={18} /></button>
                </div>
             ) : (
                <select 
